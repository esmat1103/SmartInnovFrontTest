    ...
    const fetchClients = async () => {
      try {
        ...
        const clientsMap = clientsData.reduce((acc, client) => {
          acc[client._id] = client.firstName; 
          return acc;
        }, {});
      ...
      <tbody className='darkgrey'>
        {tableData.map((row) => (
          <tr key={row._id} className="table-row-box nunito f12">
            ...
            <td className='f11 nunito pl23 center'></td>
            ...
            <td className="f11 nunito center">
              <div className={ row.status === 'In use' ? 'box-cellE f11' : row.status === 'Maintenance' ? 'box-cellH f11' : row.status === 'Suspended' ? 'box-cellD f11' : 'default-class f11'}>
                {row.status}
              </div>
            </td>
